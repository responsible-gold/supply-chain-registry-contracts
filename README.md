# Open Registry and Transfer of Custody for IoT: Smart Contracts
...
- `MultiAccessPrecise` — MulltiAccess contract, stores the owners, proxies transactions to other contracts
- `Hub` - `Hub` stores multiple models that do different things
- `ModelFactory` - `ModelFactory` constructs models. Hub is dependent on this contract.
- `CustodyController` - All the transfer of custody logic is here. 
...
### Deploy Contracts
- Clear your build directory: `rm -r build/`
- Run `truffle compile`
- Configure `migrations/2_deploy_contracts.js`'s `accounts`. `accounts` is an array of public keys. Each public key can use `MultiAccessPrecise` to proxy transactions. Make sure:
1. a private key is defined in `migrations/custom_deploy.js` 
2. the publicKey associated with the privatekey is set as `defaultAccount` in `migrations/custom_deploy.js`
3. You have three different accounts in the ethereum node are the expected owners of the multiaccess contract and they are not the same as the `defaultAccount`.
4. The rpc address and port number is correct in this line: https://github.com/responsible-gold/scr-contracts/blob/master/migrations/custom_deploy.js#L21
- Run `npm run babel`
- Launch your ethereum blockchain client
- Run `node build/custom_deploy.js` and record all console outputs


### Test contracts
Make sure to delete all contents under `migrations/` before you run tests. By default, `truffle test` will run the scripts under `migrations/` first before it runs all the tests.
```
$ testrpc
$ truffle test
```
